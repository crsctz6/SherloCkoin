...
export class CoinClient {
    ...
    getCoinsForSearch(): Promise<CoinListedSearchDTO[]> {
        let url_ = this.baseUrl + "/api/Coin/search";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetCoinsForSearch(_response);
        });
    }

    protected processGetCoinsForSearch(response: Response): Promise<CoinListedSearchDTO[]> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
            let result200: any = null;
            result200 = _responseText === "" ? null : <CoinListedSearchDTO[]>JSON.parse(_responseText, this.jsonParseReviver);
            return result200;
            });
        } else if (status !== 200 && status !== 204) {
            return response.text().then((_responseText) => {
        ...
        return Promise.resolve<CoinListedSearchDTO[]>(<any>null);
    }
...
export interface CoinListedSearchDTO {
    id?: number;
    logo?: string | undefined;
    name?: string | undefined;
